    ...
    try {
      axios.post(`${process.env.REACT_APP_BACK_ADDRESS}/basic/api/scrap/`, obj).then((res) => {
        ...
        if (res.data.status === 1) {
          setState('Here is what I found :)');
        } else {
          setState('Seems like the search did not throw any result :(');
        }
        ...
        for (let i = 0; i < aux.length; i += 1) {
          if (aux[i].priceSale === null) {
            ...
            scrappingProducts.push({
              ...aux[i],
              price: parseInt(aux[i].price.replace(/\./g, ''), 10),
              priceSale: parseInt(aux[i].priceSale.replace(/\./g, ''), 10),
            });
        }
      ...
      <Box>
        ...
        <Stack direction="column" p={10} spacing={2}>
          ...
          <Select value={category} onChange={handleChange}>
            <MenuItem value="Mujer">Women</MenuItem>
            <MenuItem value="Hombre">Man</MenuItem>
          ...
          {category !== 'Personalizada'
            ...
            : true && (
                <TextField
                  disabled={category !== 'Personalizada'}
                  id="other"
                  label="Which?"
                  value={other}
